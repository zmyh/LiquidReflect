...
public class TabGUI extends Element {
    ...
    public TabGUI() {
        for (final ModuleCategory category : ModuleCategory.values()) {
            final Tab tab = new Tab(category.getName());
    ...
    private void drawGui(final int posX, final int posY) {
        ...
        for (int i = 0; i < tabs.size(); ++i) {
            ...
            fontRenderer.drawString(tabName, getFacing().getHorizontal() == Facing.Horizontal.RIGHT ? posX + width.get() - fontRenderer.getWidth(tabName) - tabs.get(i).textFade - 3 : posX + tabs.get(i).textFade + 2, yOff + textPositionY.get(), selectedTab == i ? 0xffffff : new Color(210, 210, 210).getRGB(), textShadow.get());
    ...
    public TabGUI setFontRenderer(final CFontRenderer fontRenderer) {
    ...
    private class Tab {
        ...
        private void drawTab(int x, int y, final int color, final int backgroundColor, final int borderColor, final int borderStrength, final boolean upperCase) {
            ...
            for (final Module module : modules)
                if (fontRenderer.getWidth(upperCase ? module.getName().toUpperCase() : module.getName()) + 4 > maxWidth)
                    maxWidth = (int) (fontRenderer.getWidth(upperCase ? module.getName().toUpperCase() : module.getName()) + 7F);
