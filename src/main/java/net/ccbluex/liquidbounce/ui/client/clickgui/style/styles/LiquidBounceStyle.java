...
public class LiquidBounceStyle extends Style {
    ...
    public void drawPanel(int mouseX, int mouseY, Panel panel) {
        ...
        float textWidth = Fonts.font35.getWidth("§f" + StringUtils.stripControlCodes(panel.getName()));
    ...
    public void drawDescription(int mouseX, int mouseY, String text) {
        int textWidth = Fonts.font35.getWidth(text);

        RenderUtils.drawBorderedRect(mouseX + 9, mouseY, mouseX + textWidth + 14, mouseY + Fonts.font35.getHeight() + 3, 1, new Color(255, 255, 255, 90).getRGB(), Integer.MIN_VALUE);
        GlStateManager.resetColor();
        Fonts.font35.drawString(text, mouseX + 12, mouseY + (Fonts.font35.getHeight()) / 2, Integer.MAX_VALUE);
    }

    @Override
    public void drawButtonElement(int mouseX, int mouseY, ButtonElement buttonElement) {
        GlStateManager.resetColor();
        Fonts.font35.drawString(buttonElement.getDisplayName(), (int) (buttonElement.getX() - (Fonts.font35.getWidth(buttonElement.getDisplayName()) - 100.0f) / 2.0f), buttonElement.getY() + 6, buttonElement.getColor());
    }
    ...
    public void drawModuleElement(int mouseX, int mouseY, ModuleElement moduleElement) {
        ...
        GlStateManager.resetColor();
        Fonts.font35.drawString(moduleElement.getDisplayName(), (int) (moduleElement.getX() - (Fonts.font35.getWidth(moduleElement.getDisplayName()) - 100.0f) / 2.0f), moduleElement.getY() + 6, moduleElement.getModule().getState() ? guiColor : Integer.MAX_VALUE);
            ...
            if (moduleElement.isShowSettings()) {
                ...
                for (final Value value : moduleValues) {
                    if (value instanceof BoolValue) {
                        ...
                        final float textWidth = Fonts.font35.getWidth(text);
                        ...
                        for (final String valueOfList : listValue.getValues()) {
                            final float textWidth2 = Fonts.font35.getWidth(">" + valueOfList);
                        ...
                        final String text = value.getName() + "§f: §c" + (value instanceof BlockValue ? Block.getBlockById(integerValue.get()).getLocalizedName() + " (" + integerValue.get() + ")" : integerValue.get());
                        final float textWidth = Fonts.font35.getWidth(text);
                        ...
                        final CFontRenderer fontRenderer = fontValue.get();

                        RenderUtils.drawRect(moduleElement.getX() + moduleElement.getWidth() + 4, yPos + 2, moduleElement.getX() + moduleElement.getWidth() + moduleElement.getSettingsWidth(), yPos + 14, Integer.MIN_VALUE);

                        String displayString = "Font: Unknown";

                        if (!(fontRenderer instanceof CFontRenderer.MCFont)) {
                            final CFontRenderer liquidFontRenderer = fontRenderer;

                            displayString = "Font: " + liquidFontRenderer.getFont().getName() + " - " + liquidFontRenderer.getFont().getSize();
                        } else {
                        ...
                        int stringWidth = Fonts.font35.getWidth(displayString);
                        ...
                        if ((Mouse.isButtonDown(0) && !mouseDown || Mouse.isButtonDown(1) && !rightMouseDown) && mouseX >= moduleElement.getX() + moduleElement.getWidth() + 4 && mouseX <= moduleElement.getX() + moduleElement.getWidth() + moduleElement.getSettingsWidth() && mouseY >= yPos + 4 && mouseY <= yPos + 12) {
                            final List<CFontRenderer> fonts = Fonts.getFonts();

                            if (Mouse.isButtonDown(0)) {
                                for (int i = 0; i < fonts.size(); i++) {
                                    final CFontRenderer font = fonts.get(i);
                        ...
                        float textWidth = Fonts.font35.getWidth(text);
